...
/**
 * Implements the Resource Owner Password Credentials grant. This grant consists on two actions: 1) creating a new grant
 * allowing the client to access the resources of the RO with a certain scope; and 2) returning the newly generated token.
 */
function getResourceOwnerPasswordToken(clientId, resourceOwner, scope, callback) {
    async.series([
        async.apply(grants.add, clientId, scope, "password", resourceOwner),
        async.apply(tokenRetrieval,
            async.apply(grants.findGrantWithScopeAndResourceOwner, clientId, scope, resourceOwner),
            clientId,
            "type",
            "password"
        )
...
exports.getRoToken = getResourceOwnerPasswordToken;
